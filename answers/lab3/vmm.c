...
void
vmm_init(void) {
    swap_manager = &lru_swap_manager;  // 设置LRU替换策略
    swap_manager->init();              // 初始化LRU
    check_vmm();
}
...
do_pgfault(struct mm_struct *mm, uint_t error_code, uintptr_t addr) {
    ...
    */
    ...
    if ((ret = swap_in(mm, addr, &page)) == 0) {       // 从交换空间加载页面
        page_insert(mm->pgdir, page, addr, perm);      // 建立物理页与虚拟地址的映射
        swap_map_swappable(mm, addr, page, 1);         // 设置页面为可交换
        page->pra_vaddr = addr;                        // 更新页面虚拟地址
    } else {
